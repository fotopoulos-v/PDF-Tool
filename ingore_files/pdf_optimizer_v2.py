...
if action == "Compress":
    ...
    # Compression level slider (1-5 scale converted to DPI)
    compression_map = {
    ...
    if uploaded_file and st.button("Compress PDF"):
        ...
        if file_size_mb > 80:
            st.warning(f"⚠️ Large file ({file_size_mb:.1f} MB) - this may take a while, please be patient.")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                def update_status():
                    ...
                    status_container.markdown(f"""
                        <div style="display: flex; align-items: center; gap: 15px;">
                            ...
                            <h3 style="margin: 0;">Processing PDF... Please wait</h3>
                        </div>
                    """, unsafe_allow_html=True)
                    timer_container.caption(f"⏱️ Elapsed time: {time_str}")
                
                # Initial update
                update_status()
                
                # Use /ebook setting for compression levels 4 and 5, /screen for others
                ...
                if result.returncode == 0 and os.path.exists(output_path):
                    ...
                    output_filename = original_filename.replace(".pdf", "_compressed.pdf")
                ...
                st.error("❌ Ghostscript not found. Install it with: sudo apt install ghostscript")
...
# Extract Text
    ...
    st.write("Extract all text from your PDF and save as a PDF or text file.")
...
# Extract Pages
    ...
    st.write("Extract a range of pages from your PDF.")
    ...
    if uploaded_file:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    from_page = st.number_input("From Page", min_value=1, max_value=total_pages, value=1)
                with col2:
                    to_page = st.number_input("To Page", min_value=1, max_value=total_pages, value=total_pages)
                ...
                if st.button("Extract Pages"):
                    if from_page <= to_page:
                        ...
                        # Create output filename with page range
                        original_filename = uploaded_file.name
                        output_filename = original_filename.replace(".pdf", f"_pages_{from_page}_to_{to_page}.pdf")
...
# Split PDF
    ...
    st.header("Split PDF")
    st.write("Split a PDF into individual pages.")

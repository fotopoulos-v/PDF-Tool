            ...
            try:
                ...
                with col1:
                    with open(pdf_path, "rb") as f:
                        st.download_button("📥 Download as PDF", f.read(), "extracted_text.pdf", "application/pdf")
                with col2:
                    st.download_button("📥 Download as Text", text, "extracted_text.txt", "text/plain")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with open(zip_buffer.name, "rb") as f:
                    st.download_button("Download All Pages (ZIP)", f.read(), "split_pages.zip", "application/zip")
            except Exception as e:
                st.error(f"❌ Error: {e}")
    ...
    if uploaded_file and st.button("Rotate PDF"):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with open(output_path, "rb") as f:
                    st.download_button("Download Rotated PDF", f.read(), "rotated.pdf", "application/pdf")
            except Exception as e:
                st.error(f"❌ Error: {e}")
